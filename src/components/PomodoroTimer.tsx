...
import { Play, Pause, RotateCcw, SkipForward, Coffee, Maximize, Minimize, Keyboard } from 'lucide-react';
...
export function PomodoroTimer({
  ...
  }, [toggleTimer, resetTimer, skipPhase, toggleFullscreen, isFullscreen]);
  ...
  return (
    <div
      ...
      className={`flex flex-col items-center ${
        isFullscreen
          ? 'fixed inset-0 z-50 bg-[#010101] justify-center'
          : ''
      }`}
    >
      {/* Fullscreen header */}
      ...
      {/* Session indicators */}
      <div className={`flex items-center gap-2 ${isFullscreen ? 'mb-8' : 'mb-6'}`}>
      ...
      <div className={`relative ${isFullscreen ? 'w-96 h-96' : 'w-80 h-80'}`}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className={`font-light tracking-tight text-white tabular-nums ${isFullscreen ? 'text-8xl' : 'text-6xl'}`}>
            {formatTime(timeLeft)}
          </span>
      ...
      {/* Total work time */}
      {totalWorkTime > 0 && (
        <p className={`text-zinc-500 ${isFullscreen ? 'text-base mt-8' : 'text-sm mt-6'}`}>
      ...
      {showShortcuts && (
        <div className={`mt-4 p-4 bg-zinc-800/50 rounded-xl border border-zinc-700 ${isFullscreen ? 'text-base' : 'text-sm'}`}>
          <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-zinc-400">
            ...
            <div className="flex items-center gap-3">
              <kbd className="px-2 py-1 bg-zinc-700 rounded text-xs text-white">S</kbd>
